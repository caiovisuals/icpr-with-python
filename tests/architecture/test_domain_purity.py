...
def test_there_are_domain_modules_to_verify():
...
def test_domain_only_allowed_stdlib_and_itself(arquivo: Path):
...
@pytest.mark.parametrize("arquivo", MODULOS, ids=lambda p: p.name)
def test_domain_does_not_matter_nothing_forbidden(arquivo: Path):
...
@pytest.mark.parametrize("arquivo", MODULOS, ids=lambda p: p.name)
def test_domain_so_importa_de_app_domain(arquivo: Path):
...
def test_the_allowlist_only_contains_stdlib():
...
def test_domain_does_not_read_the_clock():
    ...
    """
    prohibited = {"now", "today", "utcnow"}
    for arquivo in MODULOS:
        arvore = ast.parse(arquivo.read_text(encoding="utf-8"), filename=str(arquivo))
        for no in ast.walk(arvore):
            if isinstance(no, ast.Call) and isinstance(no.func, ast.Attribute):
                assert no.func.attr not in prohibited, (
