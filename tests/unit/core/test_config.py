...
class TestSettings:
    ...
    def test_app_env_has_development_pattern(self, monkeypatch):
        monkeypatch.delenv("APP_ENV", raising=False)
        monkeypatch.setenv("DATABASE_URL", "postgresql+psycopg://u@h/db")
        assert Settings.from_environment().app_env == "development"

    @pytest.mark.parametrize("valor", [None, "", "   "])
    def test_database_url_absence_prevents_the_match_from_starting(self, monkeypatch, valor):
...
class TestCache:
    def test_get_settings_returns_the_same_instance(self, monkeypatch):
        monkeypatch.setenv("DATABASE_URL", "postgresql+psycopg://u@h/db")
