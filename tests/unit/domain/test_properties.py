...
@given(year=st.integers(min_value=1900, max_value=2199), month=st.integers(min_value=1, max_value=12))
@settings(max_examples=200)
def test_competencia_avanca_e_volta(year, month):
    competencia = Competencia(year, month)
    assume(competencia != Competencia(2199, 12))
    assert competencia.next().previous() == competencia


@given(year=st.integers(min_value=1900, max_value=2199), month=st.integers(min_value=1, max_value=12))
def test_competencia_faz_round_trip_pelos_dois_formatos(year, month):
    competencia = Competencia(year, month)
